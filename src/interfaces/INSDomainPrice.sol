...
import { PeriodScaler } from "src/libraries/math/PeriodScalingUtils.sol";
...
interface INSDomainPrice {
  ...
  event RenewalReservationRatioUpdated(address indexed operator, uint256 indexed ratio);
  ...
  /**
   * @dev Returns renewal reservation ratio.
   */
  function getRenewalReservationRatio() external view returns (uint256 rnfReservationRatio);
   ...
   * - The method caller is admin.
   *
   * Emits an event {RenewalReservationRatioUpdated}.
   */
  function setRenewalReservationRatio(uint256 ratio) external;
   ...
   */
  function MAX_PERCENTAGE() external view returns (uint64);
}
